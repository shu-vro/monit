//! # arpmac — macOS Internet Sharing manager
//!
//! **arpmac** is a terminal UI (TUI) that gives you router-like visibility and control
...
//! **arpmac** wraps those primitives into one keyboard-driven interface.
//!
...
//!
//! When sharing is **off**, `bridge100` does not exist and arpmac shows `INACTIVE`.
//!
//! ## Terminology
//!
//! | Term | Meaning |
...
//! | **PF / pfctl** | macOS **Packet Filter** firewall. `pfctl` loads rules; we use a dynamic **anchor** `arpmac/block`. |
...
//! | **● now / past** | arpmac labels: *now* = on bridge ARP right now; *past* = only in DHCP history. |
//!
//! ## Architecture
//!
//! ```text
...
//!         │                                                       ▼
//!         │                              pf::load_blocked()  ← ~/.config/arpmac/blocks.pf
...
//!         ├─ append IP to ~/.config/arpmac/blocks.pf
//!         └─ sudo::run(password, ["pfctl", "-a", "arpmac/block", "-f", …])
...
//! 5. **`pf::block` / `pf::unblock`** — persist rules, `pfctl -a arpmac/block`
...
//! | `~/.config/arpmac/blocks.pf` | PF rules generated by arpmac |
...
/// Run the arpmac TUI event loop until the user quits.
///
...
///
/// ```no_run
/// arpmac::run().expect("arpmac exited with an error");
/// ```
